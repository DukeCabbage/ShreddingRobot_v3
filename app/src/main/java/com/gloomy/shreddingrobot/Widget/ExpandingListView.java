...
public class ExpandingListView extends ListView {
    ...
    private final String TAG = "ExpandingListView";
    private Context _context;
    private boolean readyForMoitonEvent = false;
    ...
    public void setAdapter(ListAdapter adapter) {
        mAdapter = (HistoryArrayAdapter) adapter;
...
//            Log.e(TAG, "listView on click");
            boolean isExpanded = mAdapter.getStat(position);
            if (!isExpanded) {
                expandView(view, position);
                mAdapter.setStat(getPositionForView(view), true);
            } else {
                collapseView(view, position);
                mAdapter.setStat(getPositionForView(view), false);
            }
//            mAdapter.printAllStat();
        }
    };
     ...
     */
    private int[] getTopAndBottomTranslations(int oldTop, int oldBottom, int yDelta,
                                              boolean isExpanding) {
        int yTranslateTop = 0;
        int yTranslateBottom = yDelta;
//    	Log.e(TAG, "yDelta: "+ yDelta);
        int height = oldBottom - oldTop;

        if (isExpanding) {
            boolean isOverTop = oldTop < 0;
            boolean isBelowBottom = (oldTop + height + yDelta) > getHeight();
            if (isOverTop) {

                yTranslateTop = oldTop;
                yTranslateBottom = yDelta - yTranslateTop;
            } else if (isBelowBottom){
                int deltaBelow = oldTop + height + yDelta - getHeight();
                yTranslateTop = oldTop - deltaBelow < 0 ? oldTop : deltaBelow;
                yTranslateBottom = yDelta - yTranslateTop;
            }
        } else {
            ...
            boolean isCellCompletelyDisappearing = oldBottom - yTranslateBottom < 0;
            ...
            if (isCollapsingBelowBottom) {
                ...
                yTranslateBottom = oldBottom;
                yTranslateTop = yDelta - yTranslateBottom;
            }

            boolean isCollapsingAboveTop = (yTranslateTop > offset);
            if (isCollapsingAboveTop) {
                yTranslateTop = offset;
                yTranslateBottom = yDelta - yTranslateTop;
            }
        }
    ...
    private void expandView(final View view, final int position) {
//        Log.e(TAG, "listView expand");
        ...
        view.setBackgroundColor(getResources().getColor(R.color.history_open_BG));
        ...
        * determined, but still have not been rendered onto the screen.*/
        ...
        observer.addOnPreDrawListener(new ViewTreeObserver.OnPreDrawListener() {
            ...
            public boolean onPreDraw() {
                 ...
                 * mShouldRemove start with false, set to true to skip this block */
                if (!mShouldRemoveObserver) {
                    ...
                    * by the ListView.*/
                    ...
                    //This is the unprocess top
                    int currentTop = view.getTop();
                    //This is what the final top after animation
                ...
                *  must also be added to a list of views which will be drawn in dispatchDraw.*/
                ...
                s.addListener(new AnimatorListenerAdapter() {
                    ...
                    public void onAnimationEnd(Animator animation) {
                        ((TextView) view.findViewById(R.id.arrow_up_down)).setText(Constants.ICON_ARROW_UP);
                        setEnabled(true);
                        setClickable(true);
    ...
    private void collapseView(final View view, final int position) {
//         Log.e(TAG, "listView collapse");
        ...
        if(mAdapter.isFirstTrackOfTheDay(position)){
            int extraHeight = (int) getResources().getDimension(R.dimen.histo_category_height);
            collapsedHeight += extraHeight;
        }
    ...
    public void enableListView(boolean bool){
        readyForMoitonEvent = bool;
    }
}